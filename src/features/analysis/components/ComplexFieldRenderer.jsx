...
export default function ComplexFieldRenderer({ value, depth = 0, onChange, isAnonymized = false }) {
    ...
    if (typeof value !== 'object') {
        ...
        return (
            <input
                ...
                className="bg-transparent border-b border-white/5 text-zinc-300 focus:text-white focus:border-tivit-red/50 focus:outline-none w-full min-w-[50px] transition-colors"
                readOnly={!onChange}
            />
        );
    }
    ...
    if (Array.isArray(value)) {
        ...
        const handleArrayItemChange = (idx, newItemValue) => {
            if (!onChange) return;
    ...
    // Object Handling
    const handleObjectKeyChange = (key, newVal) => {
        if (!onChange) return;
